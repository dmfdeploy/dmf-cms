...
def create_app(settings: Settings | None = None, contract: AppContract | None = None) -> FastAPI:
    ...
    async def api_facility_summary(request: Request):
        ...
        try:
            ...
            # GATE-S1 P2a: row SHAPING lives inside the guard too. A read
            # that succeeds can still hand back a row whose `site` is null
            # or whose shape we did not predict, and reaching into it out
            # here produced exactly the 500 this endpoint was converted to
            # stop emitting. Unreachable and unparseable are both honest
            # reasons; neither is a stack trace.
            # A single unparseable row degrades ITSELF, not the page. But
            # skipping silently would make the payload claim completeness it
            # doesn't have (Art. 1), so dropping any row SETS a reason the UI
            # can show alongside the rows that did parse.
            kept_sites = [x for x in sites if isinstance(x, dict)]
            kept_devices = [x for x in devices if isinstance(x, dict)]
            skipped = (len(sites) - len(kept_sites)) + (len(devices) - len(kept_devices))
            partial_reason = "netbox-rows-unparseable" if skipped else ""
            sites, devices = kept_sites, kept_devices

            device_count_by_site: dict[str, int] = {}
            for d in devices:
                site = d.get("site")
                site_name = site.get("name", "unknown") if isinstance(site, dict) else "unknown"
    ...
    async def api_facility_devices(request: Request):
        ...
        try:
            ...
            # GATE-S1 P2a: shaping inside the guard. `site`, `device_type`
            # and `role` are all nullable in NetBox, and `.get("name")` on a
            # None was the reproduced 500 — a valid read with one null field
            ...
            devices = [
                {
                    ...
                    "name": d.get("name"),
                    "type": (
                        d.get("device_type", {}).get("model")
                        if isinstance(d.get("device_type"), dict)
                        else d.get("device_type")
                    ),
                    "site": _named(d.get("site")),
                    "status": (
                        d.get("status", {}).get("value")
                        if isinstance(d.get("status"), dict)
                        else d.get("status")
                    ),
                    "ip": (
                        d.get("primary_ip", {}).get("address")
                        if isinstance(d.get("primary_ip"), dict)
                        else None
                    ),
                    "role": _named(d.get("role")),
                }
    ...
    async def api_facility_detail(request: Request, site: str):
        """Facility Detail page (S1, #285): node identity/arch,
        platform services with as-deployed versions + real ingress
        ...
        than a hard 404. Every DATA outcome is a 200 carrying its own
        `reason` (facility.py's fail-soft contract); an unauthenticated
        request is still a 401.
        """
        if not _require_user(request):
            return JSONResponse({"error": "unauthorized"}, status_code=401)
